...
type CreateInstagramPostArgs = { file: File; onProgress?: (percent: number) => void } & Omit<
 ...
 * event — in one mutation. onProgress reports the image upload (0–100); once it
 * reaches 100 the publish/processing runs without a measurable percentage.
 */
export function useCreateInstagramPost() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      onProgress,
      ...rest
    }: CreateInstagramPostArgs): Promise<CreateEventResponse> => {
      if (!storeId) throw new Error("Store ID is required")
      const token = await getToken()
      const { url, key } = await uploadService.uploadInstagramMedia(
        file,
        storeId,
        token!,
        onProgress
